\section{Approach 3: Shares Variables}
\subsection{Mục tiêu}
Ở 2 cách trước là \textbf{Approach 1: Separate Variables} và \textbf{Approach 2: Shared Variables}, chúng ta đã tìm hiểu về cách sử dụng các biến riêng biệt và chia sẻ biến trong các mô hình học máy. Trong phần này, chúng ta sẽ khám phá cách tiếp cận thứ ba, đó là \textbf{Shares Variables} đều sử dụng biến cục bộ để đếm số điểm rơi vào hình tròn rồi cộng dồn vào tổng khi kết thúc.
...
\begin{itemize}
    \item Mỗi lần có điểm rời vào hình tròn, luồng phải thực hiện thao tác lock and unlock mutex
    \item Với số điểm lớn lên đến \(10^{7}, 10^{8}, 10^{9}\) và nhiều luồng =, thì việc lock/ unlock rất lớn dẫn đến overhead nặng nề
\end{itemize}
